...
class MotionCommand {
  ...
  static percentage(percent: number): Buffer {
    ...
    const command: string =
      MotionCommandType.PERCENT + percent_hex + '00' + MotionCrypt.getTime()
    return MotionCommand.encryptEncode(command)
  }
  ...
  static tilt(angle: number): Buffer {
    ...
    const command: string =
      MotionCommandType.ANGLE + '00' + angle_hex + MotionCrypt.getTime()
    return MotionCommand.encryptEncode(command)
  }
  ...
  static speed(speed: MotionSpeedLevel): Buffer {
    ...
    const command: string =
      MotionCommandType.SPEED + speed_hex + MotionCrypt.getTime()
    return MotionCommand.encryptEncode(command)
  }
  ...
  static statusQuery(): Buffer {
    const command: string =
      MotionCommandType.STATUS_QUERY + MotionCrypt.getTime()
    return MotionCommand.encryptEncode(command)
  }
